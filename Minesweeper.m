...
ss = sum(screen_size(3:4));N=8;bomnumber=0;
hf = figure('resize','off','name','Minesweeper',...
    'position', [450 120 900 825]*ss/3000,...
    'numbertitle', 'off','menubar','none',...%sudoku关掉是否保存数据
    'color', [205 197 191]/255);%最大画布
...
circum=[];
for counter=1:N^2
    jcols = rem(counter,N);%列，求余
    jcols(jcols==0) = N;
    irows = ceil(counter/N);%行，取整
...
for counter=1:N^2
    ...
    irows = ceil(counter/N);%行，取整
    size_squ = 50;
    position = [size_squ*jcols+59 600-size_squ*irows size_squ-2 size_squ-2]*ss/3000;
h(counter)=uicontrol( 'FontSize',18,...
            'FontWeight','bold',...
            'Position',position,...
            'Style','pushbutton',...
            'Tag',num2str(counter),...

...
```
实际上这些不同类型的`uicontrol`的用法都是类似的, 其中比如`popupmenu`, `slider`的`value`属性用于获取你选择的到底是哪一个选项或者滑动的比例. 具体也可以参见`uicontrol`的[官方文档][2]
...
这个属性是一个向量`vector`, 例如在这里你可以设置第m行第n列的`uicontrol`的`userdata`是`[m n]`就可以更加方便的对应了.
...
- [x] 重新开始游戏, 建立一个`uicontrol`, 最好是笑脸, 单击就可以重新设置炸弹的分布, [上面](#1-uicontrol显示图片)有说怎么使用`uicontrol`显示图片. 这里应该有三个图片需要准备, 第一个是笑脸, 用于重新开始游戏, 第二个是哭脸, 用于点错炸弹使用, 第三个是酷脸..., 用于赢得游戏使用.
- [x] 说到开始游戏, 以免第一次就点击到了炸弹, 可以设置在你左键点击开始的时候才分配炸弹, 这样就可以保证第一次不会碰到炸弹了
- [x] 右键单击显示小旗子, 再次右击去掉小旗子, 注意到小旗子是有限的, 使用完了就没有了, 所以需要显示小旗子的剩余量
- [x] 如果点击到了炸弹, 炸弹会全部爆出来, 注意如果你插上的小旗子是正确的, 那么那个地方的炸弹是不需要爆出来的, 其他地方的炸弹都要显示出来, 错误的小旗子可以显示一个红色的背景? 或者其他的以示区分
- [x] 当然还需要有计时功能, 这个详解在[这里](./timeui.md). 有了计时之后可能就需要做一个排行榜的东西...
- [x] 接着就是设置难度这个东西, 其实最初已经做好了一定的准备, 就是可以灵活的设置方块的个数就可以控制难度了. 一般可以设置成这样, 难度默认有简单, 中等和困难三个模式, 也可以自定义`N`(当然这个`N`需要有一定范围, 不能太大), 这个可以设置成下拉菜单的模式, 也就是`popumenu`这个类型的`uicontrol`, 这个[类型](#2-popupmenu)在上面有讲到.
